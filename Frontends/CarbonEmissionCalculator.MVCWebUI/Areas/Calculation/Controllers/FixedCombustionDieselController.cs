...
namespace CarbonEmissionCalculator.MVCWebUI.Areas.Calculation.Controllers
{
    ...
    public class FixedCombustionDieselController : Controller
    {
        ...
        public FixedCombustionDieselController(IUnitOfWork unitOfWork, ICustomMapper customMapper, CompanyService companyService)
        {
        ...
        {
            IList<FixedCombustionDieselCalculation> values = await _unitOfWork.GetReadRepository<FixedCombustionDieselCalculation>().GetAllAsync(include: v => v.Include(v => v.Company));

            return View(values);
        }
        public async Task<IActionResult> Detail(int id)
        {
            FixedCombustionDieselCalculation value = await _unitOfWork.GetReadRepository<FixedCombustionDieselCalculation>().GetAsync(x => x.Id == id, include: x => x.Include(x => x.Company));
