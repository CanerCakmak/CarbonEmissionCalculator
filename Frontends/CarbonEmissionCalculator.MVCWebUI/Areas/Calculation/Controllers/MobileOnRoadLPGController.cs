...
namespace CarbonEmissionCalculator.MVCWebUI.Areas.Calculation.Controllers
{
    ...
    public class MobileOnRoadLPGController : Controller
    {
        ...
        public MobileOnRoadLPGController(IUnitOfWork unitOfWork, ICustomMapper customMapper, CompanyService companyService)
        {
        ...
        {
            IList<MobileOnRoadLPGCalculation> values = await _unitOfWork.GetReadRepository<MobileOnRoadLPGCalculation>().GetAllAsync(include: v => v.Include(v => v.Company));

            return View(values);
        }
        public async Task<IActionResult> Detail(int id)
        {
            MobileOnRoadLPGCalculation value = await _unitOfWork.GetReadRepository<MobileOnRoadLPGCalculation>().GetAsync(x => x.Id == id,include:x=>x.Include(x=>x.Company));
